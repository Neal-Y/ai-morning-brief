...
- the button row now uses absolute positioning in the extended outer app layer with `bottom: 24px`, instead of fixed positioning against the clipped dynamic viewport
...
- local working-tree experiment on 2026-04-28, current
  ...
  - positions the button row as `position: absolute; bottom: 24px` inside that extended outer layer
  - `bottom: 12px` successfully entered the bottom area but sat slightly too low on the real device, so it was raised by 12px
